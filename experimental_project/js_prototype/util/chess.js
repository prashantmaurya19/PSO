 ...
 * @param {Partial<Record<MotionType,boolean>>} [exclude_motion_type={}]
 * @returns {AttackingPieceInfo}
 */
export function getAllAttackPiece(index_info, exclude_motion_type = {}) {
  /**
   * @type {AttackingPieceList}
   */
  const res = [];
  for (const motion_type in Handler.IS_PIECE_ATTACKING) {
    if (exclude_motion_type[motion_type]) continue;
