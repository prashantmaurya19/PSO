      ...
      istralingspace =
        (j == to[0] && i == to[1]
          ? isSameColorPiece(from_piece, piece)
            ? false
            : true
          : piece == EMPTY_FEN_CHAR) && istralingspace;
      res.placeable =
        (j == to[0] && i == to[1] && istralingspace) || res.placeable;
 ...
 * @param {function(number,number,FenChar|""):T} callback
 * @returns {Array<T>}
 */
 ...
 */
export function getMoveInfo(from, to, position) {
  const piece = getPieceAt(...from, position);
  return piece == EMPTY_FEN_CHAR
    ? undefined
    : Motion[piece] == undefined
      ? undefined
      : Motion[piece](from, to, position);
}
 ...
 * @returns {{fen:FenString,transition_info:TransitionInfo|undefined}}
 */
export function transition(from, to, fen) {
  const info = parse(fen);
  const move_info = getMoveInfo(from, to, info.position);
  if (move_info == undefined || !move_info.placeable) return { fen };
  return {
    transition_info: {
      capture: {
    ...
    fen: stringify({
      position: putPieceAt(
        info_from.piece,
        ...to,
        pickPieceAt(...from, info.position),
      ),
...
export function print(position) {
  let res = "";
  streamFenIndexInfo(position, (i, _, piece) => {
    res = res.concat(
