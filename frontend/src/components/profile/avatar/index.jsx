...
export function AvatarCircle({ avatarProps, className = "", ...a }) {
  return (
    <div className={twMerge(joinTWClass("aspect-square"), className)} {...a}>
      <Avatar {...avatarProps} />
    </div>
  );
}
 ...
 */
export function Avatar({ className, src = "", ...a }) {
  if (src == "") {
    return (
      <svg
        {...a}
        className={twMerge(joinTWClass("w-full h-full"), className)}
  ...
  return (
    <img
      className={twMerge(joinTWClass("w-full h-full bg-cover"), className)}
      src={src}
    />
  );
}
