 ...
 * @typedef {MotionMap<Object<string,MotionPathInfo>> & {piece:FenChar}} IndexInfo
...
/**
 * @template T
 * @typedef {Record<"diagonal"|"straight"|"knight",T>} MotionMap
 */
  ...
  static k(from, to, info) {
    ...
    if (
      !(
        ...
        isSafeFor(getPieceAt(...from, info.position), to, info.position)
      )
    ) {
      return this.getEmptyMovePathInfo();
    }
    ...
    if (
      ...
      isSafePath(from, to, info.position, { from: false, to: false })
    ) {
...
class Handler {
  ...
  static PLAYED_MOTION_PATH_INFO = {
    place: function ({ move_info, fen_info, king_info, new_fen_info }) {
      ...
      pmlog(
        move_info,
        isSafeAt(king_info[fen_info.color], new_position),
        fen_info,
      );
      if (
  ...
  /**
   * @type {MotionMap<function(MotionPathInfo):undefined|FenChar>}
   */
  static IS_PIECE_ATTACKING = {
    ...
    diagonal: function (info) {
      ...
      if (piece == "q" || piece == "b") {
        ...
        piece == "p" &&
        info.empty_space == 0 &&
        MotionPath[`isPawnCapture${info.blocking_piece.piece}`](info.motion)
      ) {
        return info.blocking_piece?.piece;
      }
    },
...
/** return array of piecies that can attak
 * @param {IndexInfo} index_info
 * @returns {AttackingPieceInfo}
 */
export function getAllAttackPiece(index_info) {
  /**
  ...
  for (const motion_type in Handler.IS_PIECE_ATTACKING) {
    for (const i in index_info[motion_type]) {
      const attack_piece = Handler.IS_PIECE_ATTACKING[motion_type](
        index_info[motion_type][i],
      );
      if (attack_piece) {
        pmlog(
          "getAllAttackPiece",
          motion_type,
          index_info[motion_type][i].blocking_piece,
        );
        res.push(attack_piece);
      }
    }
  }
  return {
 ...
 * @returns {boolean}
 */
export function isSafeAt(index, position) {
  pmlog(
    "isSafeAt",
    getAllAttackPiece(getIndexInfoInAllMotion(index, position)),
    getIndexInfoInAllMotion(index, position),
  );
  return isSafeIndex(
    getAllAttackPiece(getIndexInfoInAllMotion(index, position)),
  );
 ...
 * @returns {boolean}
 */
export function isSafeFor(piece, index, position) {
  pmlog(piece);
  return isSafeIndexFor(
    piece,
    getAllAttackPiece(getIndexInfoInAllMotion(index, position)),
  );
}
...
/**
 * @param {FenCharWithEmptyFen} p
 * @param {AttackingPieceInfo} index_info
 * @returns {boolean}
 */
export function isSafeIndexFor(p, index_info) {
  for (const piece of index_info.list) {
    if (!isSameColorPiece(p, piece)) return false;
  }
  return true;
}
 ...
 * @returns {boolean}
 */
export function isSafeIndex(index_info) {
  for (const piece of index_info.list) {
    if (!isSameColorPiece(index_info.piece, piece)) return false;
  }
  return true;
}
 ...
 * @returns {IndexInfo}
 */
export function getIndexInfoInAllMotion(index, fen) {
  /**
   * @type {IndexInfo}
   */
  ...
  for (const path_type in MotionPath.PATHS) {
    for (const motion of MotionPath.PATHS[path_type]) {
      res[path_type][motion] = getPathInfoByMotion(index, to, motion, fen);
